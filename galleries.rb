  ...
  class GalleryTag < Liquid::Block
    ...
    def gen_gallery_data_from_item(item, source_dir, cap_ref, itemnum)
      ...
      unless cap_ref.nil?
        ...
        cap << " (#{itemnum})" unless itemnum.nil?
      end
    ...
    def block_contents(context)
      ...
      if (text.nil? || text.empty?) && !nl[1].nil? # check if we have a variable in the nodelist
        text = nl[1].render(context) # render the variable
          end
  ...
  class ThumbGenerator < Generator
    ...
    def files_to_resize(site)
      ...
      Dir.glob(File.join(@gallery_dir, '**', '*.{png,jpg,jpeg,gif}')).each do |file|
        next if File.basename(file).include? '-thumb'
    ...
    def thumbify(items)
      ...
      if items.count > 0
        items.each do |item|
          ...
          unless Dir.exist?(File.dirname(item['thumbname']))
